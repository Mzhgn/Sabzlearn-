...
const globalSearch = async () => {
  ...
  const searchResultWrapper = document.querySelector("#courses-container");
  const articlesResultWrapper = document.querySelector("#articles-wrapper");

  const res = await fetch(`http://127.0.0.1:4000/v1/search/${searchValue}`);
  const data = await res.json();
  if (data.allResultCourses.length) {
    data.allResultCourses.forEach((course) => {
      searchResultWrapper.insertAdjacentHTML(
        "beforeend",
        `
        <div class="col-4">
                     <a href="course.html?name=${course.shortName}">
                    ...
                    <div class="course-box__main">
                      ...
                      <div class="course-box__status">
                        <div class="course-box__users">
                          <i class="fas fa-users course-box__users"></i>
                          <span class="course-box__users-text">5</span>
                        </div>
                        <span class="course-box__price">${
                          course.price === 0
                            ? "رایگان"
    ...
    searchResultWrapper.insertAdjacentHTML(
      "beforeend",
      `<div class="alert alert-danger">متاسفانه دوره ای برای جستجو شما یافت نشد</div>`
    );
  }
  if (data.allResultArticles.length) {
    data.allResultArticles.forEach((article) => {
      articlesResultWrapper.insertAdjacentHTML(
        "beforeend",
        `
      <div class="col-4">
              <div class="article-card">
    ...
    articlesResultWrapper.insertAdjacentHTML(
      "beforeend",
      `<div class="alert alert-danger">متاسفانه مقاله ای برای جستجو شما یافت نشد</div>`
    );
  }

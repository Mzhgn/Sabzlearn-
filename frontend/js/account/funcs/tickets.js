...
const showAllUserTickets = async () => {
  ...
  const res = await fetch(`http://127.0.0.1:4000/v1/tickets/user`, {
    headers: {
  ...
  if (tickets.length) {
    tickets.forEach((ticket) => {
      ticketsListWrapper.insertAdjacentHTML(
        "beforeend",
        `
                <div class="ticket-content__box">
                    <div class="ticket-content__right">
                        <div class="ticket-content__right-right">
                            <a class="ticket-content__link" href="../Ansewr-Ticket/index.html?id=${
                              ticket._id
                            }">${ticket.title}</a>
                    ...
                    <div class="ticket-content__left">
                        <div class="ticket-content__left-right">
                            <div class="ticket-content__condition">
                                <span class="ticket-content__condition-text">پاسخ داده ${
                                  ticket.answer === 1 ? "شده" : "نشده"
                                }</span>
                            </div>
                        </div>
                        <div class="ticket-content__left-left">
                            <span class="ticket-content__time">${ticket.createdAt.slice(
                              0,
                              10
                            )}</span>
                        </div>

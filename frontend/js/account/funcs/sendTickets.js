...
const prepareSendTicketForm = async () => {
  ...
  const ticketsSubDepartmentsWrapper =
    document.querySelector("#sub-departments");
  ...
  const res = await fetch(`http://127.0.0.1:4000/v1/tickets/departments`);
  ...
  ticketsDepartmentsWrapper.addEventListener("change", (event) => {
    ...
    fetch(
      `http://127.0.0.1:4000/v1/tickets/departments-subs/${event.target.value}`
    )
      .then((res) => res.json())
      .then((subDepartment) => {
  ...
  const userCoursesRes = await fetch(
    `http://127.0.0.1:4000/v1/users/courses/`,
    {
      headers: {
...
const sendTicket = async () => {
  ...
  const ticketBodyInput = document.querySelector("#ticket-body");
  ...
  const res = await fetch(`http://127.0.0.1:4000/v1/tickets`, {
    method: "POST",
    headers: {
